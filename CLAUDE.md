...
- **`hecras_gui_runner.py`** — Tkinter GUI wrapping the same logic
  - File browsers for project/DSS selection
  - Per-plan suffix configuration
  - Parallel/sequential toggle, temp cleanup option
  - Threaded execution with log panel
...
- Python stdlib: `os`, `time`, `shutil`, `tempfile`, `multiprocessing`, `tkinter`
